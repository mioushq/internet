...
import type { Plan, Operator, Media } from '@/payload-types';

interface OfferCardProps {
  plan: Plan;
}

export default function OfferCard({ plan }: OfferCardProps) {
  // Bezpieczne rozpakowanie operatora (przy depth=1 to obiekt, inaczej to string id)
  const operator = typeof plan.operator === 'object' && plan.operator !== null 
    ? (plan.operator as Operator) 
    : null;

  // Bezpieczne rozpakowanie logo (może być Media lub id)
  const logoUrl = operator && typeof operator.logo === 'object' && operator.logo !== null 
    ...
    <div className="glass-panel rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden transition-all hover:bg-white/15 border-white/10">
      ...
      {plan.display?.badge && (
        <div 
          ...
          style={{ backgroundColor: plan.display.badgeColor || '#06b6d4', color: '#fff' }}
        >
          {plan.display.badge}
        </div>
      )}
      ...
      <div className="flex-1 text-center md:text-left">
        ...
        <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm mt-4">
          {plan.speeds?.download && (
            <div className="flex items-center gap-1 text-cyan-400 font-medium">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>
              <span className="text-base">{plan.speeds.download} Mbps</span>
            </div>
          )}
          {plan.contract?.months && plan.contract.months !== '0' && (
            <div className="flex items-center gap-1 text-gray-300">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              <span>{plan.contract.months} msc</span>
            </div>
          )}
          {plan.contract?.months === '0' && (
            <div className="flex items-center gap-1 text-gray-300">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              <span>Bez lojalki</span>
            </div>
          )}
      ...
      <div className="flex flex-col items-center md:items-end min-w-[150px] border-t md:border-t-0 md:border-l border-white/10 pt-4 md:pt-0 md:pl-6 w-full md:w-auto mt-4 md:mt-0">
        <div className="text-3xl font-bold text-white mb-1">
          {plan.pricing?.priceMonthly} <span className="text-base text-gray-400 font-normal">zł/mc</span>
        </div>
        {plan.pricing?.activationFee && plan.pricing.activationFee > 0 ? (
          <div className="text-xs text-gray-400 mb-4">
            Aktywacja: {plan.pricing.activationFee} zł
          </div>
        ) : (
        ...
        >
          {plan.display?.customCTA || "Wybierz ofertę"}
        </Link>
      </div>
    </div>
  );
